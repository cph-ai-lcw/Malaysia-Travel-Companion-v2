...
function header(){return `<div class="topbar"><div class="brand"><div class="brand-mark">🇲🇾</div><div><b>${t('馬來西亞手帳 2026','Sổ tay Malaysia 2026')}</b><small>國能員旅 · Release v3.8</small></div></div><button class="lang" id="langBtn">${state.lang==='zh'?'VI':'中'}</button></div>`}
function nav(active){const items=[['home','🏠',t('首頁','Trang chủ')],['itinerary','🗓️',t('行程','Lịch trình')],['budget','💰',t('費用','Chi phí')],['traveler','👥',t('團員','Thành viên')],['guide','📖',t('手冊','Cẩm nang')],['map','🗺️',t('地圖','Bản đồ')],['notifications','🔔',t('通知','Thông báo')],['checkin','✅',t('簽到','Điểm danh')],['countdown','⏳',t('倒數','Đếm ngược')],['weather','🌦️',t('天氣','Thời tiết')],['exchange','💱',t('匯率','Tỷ giá')],['shopping','🛍️',t('購物','Mua sắm')]];return `<nav class="nav"><div class="nav-inner">${items.map(([r,i,l])=>`<a href="#/${r}" class="${active===r?'active':''}"><i>${i}</i>${l}</a>`).join('')}</div></nav>`}
...
function render(){if(countdownInterval){clearInterval(countdownInterval);countdownInterval=null}const r=route();const map={home,itinerary,budget,food,shopping:shoppingPage,resort,traveler,rooms,guide,map:mapPage,notifications:notificationsPage,checkin:checkinPage,countdown:countdownPage,weather:weatherPage,exchange:exchangePage};document.getElementById('app').innerHTML=map[r]();bind(r);if(r==='weather'&&!state.weatherData&&!state.weatherLoading&&!state.weatherError){setTimeout(()=>loadWeather(),0)}if(r==='exchange'&&!state.exchangeRate&&!state.exchangeLoading){setTimeout(()=>loadExchange(),0)}if(r==='home'||r==='countdown'){countdownInterval=setInterval(updateCountdownDom,1000)}}
function bind(r){
...
if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=380').catch(console.warn))}
