(function(global){'use strict';const modules={},cache={};
...
function require(id){if(cache[id])return cache[id].exports;if(!modules[id])throw new Error('QR module '+id+' missing');const m=cache[id]={exports:{}};modules[id](m,m.exports,require);return m.exports;}const QRCode=require('index'),Level=require('QRErrorCorrectLevel');global.AMTQRCode={render(canvas,text,size){const qr=new QRCode(-1,Level.M);qr.addData(String(text));qr.make();const count=qr.getModuleCount(),quiet=4,total=count+quiet*2,px=Math.max(2,Math.floor((size||220)/total));canvas.width=canvas.height=total*px;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#111';for(let r=0;r<count;r++)for(let c=0;c<count;c++)if(qr.isDark(r,c))ctx.fillRect((c+quiet)*px,(r+quiet)*px,px,px);return canvas;}};})(window);